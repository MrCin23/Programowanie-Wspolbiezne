...
namespace Logic
{
    public class Board
    {
        ...
        public void checkBorderCollision() 
        {
            ...
            {
                if (ball.x + ball.getSize() >= this.sizeX || ball.x + ball.getXVelocity() + ball.getSize() >= this.sizeX || ball.x + ball.getSize() <= 0 || ball.x + ball.getXVelocity() + ball.getSize() >= 0)
                {
                    Logic.changeXdirection(ball);
                    Logic.updatePosition(ball);
                }
                if (ball.y + ball.getSize() >= this.sizeY || ball.y + ball.getYVelocity() + ball.getSize() >= this.sizeY || ball.y + ball.getSize() >= 0 || ball.y + ball.getYVelocity() + ball.getSize() >= 0)
                {
